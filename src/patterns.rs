...
use std::fmt;
use std::ptr;
use {Matrix, MatrixTrait, Path, Surface};
    ...
    unsafe fn from_raw_full(pointer: *mut cairo_pattern_t) -> Pattern {
        ...
        match pattern_type {
            ...
            PatternType::LinearGradient => {
                Pattern::LinearGradient(LinearGradient::from_raw_full(pointer))
            }
            PatternType::RadialGradient => {
                Pattern::RadialGradient(RadialGradient::from_raw_full(pointer))
            }
...
pub trait PatternTrait {
    ...
    fn status(&self) -> Status {
        unsafe { Status::from(ffi::cairo_pattern_status(self.as_ptr())) }
    }

    fn get_reference_count(&self) -> isize {
        unsafe { ffi::cairo_pattern_get_reference_count(self.as_ptr()) as isize }
    }

    fn set_extend(&self, extend: Extend) {
        unsafe { ffi::cairo_pattern_set_extend(self.as_ptr(), extend.into()) }
    }

    fn get_extend(&self) -> Extend {
        unsafe { Extend::from(ffi::cairo_pattern_get_extend(self.as_ptr())) }
    }

    fn set_filter(&self, filter: Filter) {
        unsafe { ffi::cairo_pattern_set_filter(self.as_ptr(), filter.into()) }
    }

    fn get_filter(&self) -> Filter {
        unsafe { Filter::from(ffi::cairo_pattern_get_filter(self.as_ptr())) }
    }

    fn set_matrix(&self, matrix: Matrix) {
        unsafe { ffi::cairo_pattern_set_matrix(self.as_ptr(), &matrix) }
    }
...
impl SolidPattern {
    pub fn from_rgb(red: f64, green: f64, blue: f64) -> SolidPattern {
        unsafe { SolidPattern::from_raw_full(ffi::cairo_pattern_create_rgb(red, green, blue)) }
    }

    pub fn from_rgba(red: f64, green: f64, blue: f64, alpha: f64) -> SolidPattern {
        unsafe {
            SolidPattern::from_raw_full(ffi::cairo_pattern_create_rgba(red, green, blue, alpha))
        }
    }
...
pub trait Gradient: PatternTrait {
    fn add_color_stop_rgb(&self, offset: f64, red: f64, green: f64, blue: f64) {
        unsafe { ffi::cairo_pattern_add_color_stop_rgb(self.as_ptr(), offset, red, green, blue) }
    }
...
impl LinearGradient {
    pub fn new(x0: f64, y0: f64, x1: f64, y1: f64) -> LinearGradient {
        unsafe { LinearGradient::from_raw_full(ffi::cairo_pattern_create_linear(x0, y0, x1, y1)) }
    }
...
impl RadialGradient {
    pub fn new(x0: f64, y0: f64, r0: f64, x1: f64, y1: f64, r1: f64) -> RadialGradient {
        unsafe {
            RadialGradient::from_raw_full(ffi::cairo_pattern_create_radial(x0, y0, r0, x1, y1, r1))
        }
    }
...
impl SurfacePattern {
    pub fn create<T: AsRef<Surface>>(surface: &T) -> SurfacePattern {
        unsafe {
            SurfacePattern::from_raw_full(ffi::cairo_pattern_create_for_surface(
...
impl Mesh {
    pub fn new() -> Mesh {
        unsafe { Mesh::from_raw_full(ffi::cairo_pattern_create_mesh()) }
    }
