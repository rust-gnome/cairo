...
impl Context {
    ...
    pub fn new<T: AsRef<Surface>>(target: &T) -> Context {
        unsafe { Self::from_raw_full(ffi::cairo_create(target.as_ref().to_raw_none())) }
    }
    ...
    pub fn set_source_surface<T: AsRef<Surface>>(&self, surface: &T, x: f64, y: f64) {
        unsafe {
            ffi::cairo_set_source_surface(self.0, surface.as_ref().to_raw_none(), x, y);
        }
    }
    ...
    pub fn mask(&self, pattern: &Pattern) {
        unsafe { ffi::cairo_mask(self.0, pattern.as_ptr()) }
    }
    ...
    pub fn set_font_matrix(&self, matrix: Matrix) {
        unsafe { ffi::cairo_set_font_matrix(self.0, &matrix) }
    }
...
mod tests {
    ...
    fn drop_non_reference_pattern() {
        let ctx = create_ctx();
        let mut pattern =
            Pattern::LinearGradient(LinearGradient::new(1.0f64, 2.0f64, 3.0f64, 4.0f64));
