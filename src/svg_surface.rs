...
impl SVGSurface {
    pub fn from(surface: Surface) -> Result<SVGSurface, Surface> {
    ...
    #[doc(hidden)]
    pub unsafe fn from_raw_full(ptr: *mut ffi::cairo_surface_t) -> SVGSurface {
        Self::from(Surface::from_raw_full(ptr)).unwrap()
    }

    pub fn create<T: AsRef<Path>>(filename: T, width: f64, height: f64) -> SVGSurface
    {
        // Convert: AsRef<Path> -> Cow<str> -> str
        let s = filename.as_ref().to_string_lossy().into_owned();
        let file = CString::new(s).unwrap();
        unsafe { Self::from_raw_full(cairo_svg_surface_create(file.as_ptr(), width, height)) }
    }
}
...
impl Clone for SVGSurface {
    fn clone(&self) -> SVGSurface {
        SVGSurface(self.0.clone())
    }
}

unsafe impl Send for SVGSurface {}
...
mod tests {
    ...
    fn svg() {
        ...
        { //Cow<str>
            let filename = output.join("test1.svg");
            let surface = SVGSurface::create(filename, 100., 100.);
            let cr = Context::new(&surface);
            draw_x(&cr);
        }
        { // &str
            let filename = output.join("test2.svg");
        ...
        { // String
            let filename = output.join("test3.svg");
            let s = match filename.to_str() {
                None => panic!("Error converting Path to String"),
